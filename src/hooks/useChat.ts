 ...
 *
 * Architecture: Currently uses mock streaming for local development.
 * When Supabase Edge Function is deployed, swap the sendToAI() call
 * to hit the real endpoint with Vercel AI SDK's useChat or manual SSE.
 */
...
export function useChat(): UseChatReturn {
  ...
  const dogContext: DogContext = {
    ...
    ageWeeks: onboarding.ageMonths ? Math.round(onboarding.ageMonths * 4.3) : 12,
    developmentalStage: "", // filled by prompt builder
  ...
  const sendMessage = useCallback(
    async (content: string) => {
      ...
      // Create assistant message (streaming placeholder)
      const assistantMsg = addAssistantMessage("");

      // Send to AI (mock for now, real integration later)
      try {
        await sendToAI(
          systemPrompt,
          recentMsgs,
...
// ──────────────────────────────────────────────
// AI Integration — Mock for now
//
// TODO: Replace with real Vercel AI SDK / Edge Function call:
//   POST /api/chat/message → SSE stream
//
// The mock simulates streaming with realistic Buddy-style responses
...
async function sendToAI(
  ...
  // Simulate streaming with a natural typing feel
  let streamed = "";
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    // Variable delay for natural feel (faster for common words, slower for punctuation)
...
function generateMockResponse(dogName: string, userMessage: string): string {
  ...
  // Potty training
  if (lower.includes("potty") || lower.includes("accident") || lower.includes("pee") || lower.includes("poop")) {
  ...
  // Biting/nipping
  if (lower.includes("bit") || lower.includes("nip") || lower.includes("chew") || lower.includes("mouth")) {
  ...
  // Jumping
  if (lower.includes("jump") || lower.includes("guest") || lower.includes("visitor")) {
  ...
  // Crate training
  if (lower.includes("crate") || lower.includes("kennel") || lower.includes("whining") || lower.includes("cry")) {
  ...
  // Leash/walking
  if (lower.includes("leash") || lower.includes("pull") || lower.includes("walk")) {
  ...
  if (lower.includes("learned") || lower.includes("did it") || lower.includes("first time") || lower.includes("great day") || lower.includes("good")) {
  ...
  // Medical keywords → escalation
  if (lower.includes("vomit") || lower.includes("blood") || lower.includes("limp") || lower.includes("poison") || lower.includes("chocolate") || lower.includes("sick")) {
  ...
  if (lower.includes("work on today") || lower.includes("today's training") || lower.includes("what should")) {
